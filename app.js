...
// API endpoint to get only the token
app.post('/get-token', async (req, res) => {
  ...
  try {
    ...
    // Get only the token (without the public key) using the getToken function
    const { token } = await Promise.race([getToken(email, password), timeoutPromise]);
